...
SERVICE_ID      = 'roto-resource-service'
DASHBOARD_API   = os.environ.get('DASHBOARD_API', 'http://localhost:3001')
...
def report_health(status: str, started_at: str, records: int = 0, error: str = None, duration: int = None):
    """POST run result to CZON Dashboard pipeline health endpoint. Never raises."""
    try:
        payload = json.dumps({
        ...
        req = urllib.request.Request(
            f'{DASHBOARD_API}/api/pipeline/health',
            data=payload,
            headers={'Content-Type': 'application/json'},
            method='POST',
        )
        urllib.request.urlopen(req, timeout=10)
        log.info('[health] reported %s → %s', SERVICE_ID, status)
    except Exception as exc:
        log.warning('[health] report failed (non-fatal): %s', exc)
...
async def main():
    ...
    errors     = 0
    error_msg  = None
    status     = 'fail'  # default until proven otherwise
    ...
    try:
        ...
        # Only mark pass when the harvest loop completes without exception
        status = 'pass' if errors == 0 else 'fail'
        if errors > 0:
            error_msg = f'{errors} download(s) failed after retries'
        ...
        status    = 'fail'
        error_msg = str(exc)
        log.error('=== Roto harvest FAILED ===')

    finally:
        duration = int(time.monotonic() - t0)
        report_health(status, started_at, records=saved, error=error_msg, duration=duration)
